...
namespace {
  ...
  class AFLCoverage : public ModulePass {
    ...
    public:
      ...
      void global_check_insert(Value *target, Module &M);
...
void AFLCoverage::global_check_insert(Value *target, Module &M)
{
    std::queue<Value*> q;
    q.push(target);
    while(!q.empty())
    {
      Value *cur = q.front();
      for(Value::user_iterator iter = cur->user_begin(); iter != cur->user_end(); iter++)
      {
        Value *user = *iter;
        //q.push(user);
        if(this->visited.find(user)!=this->visited.end())
          continue;
        this->visited.insert(user);
        if(StoreInst *si = dyn_cast<StoreInst>(&*user))
        {
          ...
          if(po==cur){
            ...
            auto * helperTy = FunctionType::get(voidTy, {int8PtrTy,Int32Ty}, false);
            auto global_store = M.getOrInsertFunction("check_global_store",helperTy);
            IRBuilder<> builder(si);
            builder.SetInsertPoint(si->getNextNode());
            builder.CreateCall(global_store, {po,ConstantInt::get(Int32Ty,si->getAlignment())});
          }
        }
        else
          q.push(user);
...
bool AFLCoverage::runOnModule(Module &M) {
  ...
  for (auto &F : M){
    for (auto &BB : F) {
      ...
      for(BasicBlock::iterator iter=BB.begin(); iter!=BB.end(); iter++){
        CallInst * call_instr=dyn_cast<CallInst>(iter);
        ...
        else if(called_func->getName() == "calloc"){
          auto * voidTy = Type::getVoidTy(M.getContext());
          auto * int8PtrTy = Type::getInt8PtrTy(M.getContext());
          auto * int32Ty = Type::getInt32Ty(M.getContext());
        ...
        else if(called_func->getName() == "realloc"){
          auto * voidTy = Type::getVoidTy(M.getContext());
          auto * int8PtrTy = Type::getInt8PtrTy(M.getContext());
          auto * int32Ty = Type::getInt32Ty(M.getContext());
  ...
  {
    if(!gv.getName().equals("__afl_prev_loc") && !gv.getName().equals("__afl_area_ptr")){
      //printf("%s\n", gv.getName().data());
      global_check_insert((Value*)&gv, M);
    }
  }
